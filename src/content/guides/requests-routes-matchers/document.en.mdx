...
In modern browsers, the History API lets applications change the current URL *without* reloading the page or sending a new Request. Navi uses this API to keep track of the latest URL, and for programmatic navigation. In fact, for simple apps, you wouldn't even really need Navi for this -- you could just use a simple wrapper around the History API, like the [history](https://www.npmjs.com/package/history) package used by react-router:
...
Navi is able to access the browser history through the same [history](https://www.npmjs.com/package/history) package as react-router, smoothing integration between the two libraries. To make use of this feature, just pass a `history` prop to your `<Router>` or `createBrowserNavigation()` call.
