...
Navi is designed to work alongside react-router. This is made possible because react-router interacts with the browser history API using a separate [history](https://www.npmjs.com/package/history) package -- which can be shared with Navi by passing it to your `<Router>` component or `createBrowserNavigation()` call.
...
function Layout({ basename, children }) {
    ...
    <div className="App">
      ...
      <header className="App-header">
        <h1 className="App-title">
          <Link href='/' style={{float: 'right'}} prefetch={null}>
            <span>Back to app &raquo;</span>
          </Link>
          <Link href={basename}>
            <span>Support</span>
          </Link>
